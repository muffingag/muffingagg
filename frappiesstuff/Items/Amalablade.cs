using Terraria.ID;
using Terraria.ModLoader;

namespace frappiesstuff.Items
{
	public class Amalablade : ModItem
	{
		public override void SetStaticDefaults() 
		{
			// DisplayName.SetDefault("asd"); // By default, capitalization in classnames will add spaces to the display name. You can customize the display name here by uncommenting this line.
			Tooltip.SetDefault("Eugh- so many weak blades combined into one powerful one! Must be useful for something else though, right?");
		}

		public override void SetDefaults() 
		{
			item.damage = 80;
			item.melee = true;
			item.width = 40;
			item.height = 40;
			item.useTime = 5;
			item.useAnimation = 5;
			item.useStyle = 1;
			item.knockBack = 600;
			item.value = 10000;
			item.rare = 2;
			item.UseSound = SoundID.Item1;
			item.autoReuse = true;
		}

		public override void AddRecipes() 
		{
			ModRecipe recipe = new ModRecipe(mod);
			recipe.AddIngredient(ItemID.WoodenSword, 1);
			recipe.AddIngredient(ItemID.BorealWoodSword, 1);
			recipe.AddIngredient(ItemID.RichMahoganySword, 1);
			recipe.AddIngredient(ItemID.ShadewoodSword, 1);
			recipe.AddIngredient(ItemID.EbonwoodSword, 1);
			recipe.AddIngredient(ItemID.PearlwoodSword, 1);
			recipe.AddIngredient(ItemID.PalmWoodSword, 1);
			recipe.AddIngredient(ItemID.PalmWoodSword, 1);
			recipe.AddIngredient(mod.ItemType("Wewd"), 10);
			recipe.AddTile(TileID.WorkBenches);
			recipe.SetResult(this);
			recipe.AddRecipe();
		}
	}
}